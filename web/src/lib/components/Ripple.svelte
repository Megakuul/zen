<script>
  ...
  const titles = ['Unleash', 'your', 'Time', ' ⌛ '];
      ...
      {#key title}
        <button
          ...
          style="width: {mainCircleSize + i * 70}px;
            ...
            border-style: solid;
            border-width: 1px;
            border-color: rgba(var(--foreground-rgb), {(5 + i * 5) / 100});">{title}</button
        >
      {/key}
    {:else}
      <div
        class="circle absolute rounded-full bg-slate-700/80 shadow-xl border [--i:{i}]"
        style="width: {mainCircleSize + i * 70}px;
          height: {mainCircleSize + i * 70}px;
          opacity: {mainCircleOpacity - i * 0.03}; 
          animation-delay: {i * 0.08}s;
...
<style>
  .circle {
    top: 50%;
    left: 50%;
    animation: animate-ripple 2s ease calc(var(--i, 0) * 0.2s) infinite;
  }
