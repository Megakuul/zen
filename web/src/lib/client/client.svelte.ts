...
let transport: Transport = $derived(
...
let authTransport: Transport = $derived(
  createConnectTransport({
    baseUrl: transportUrl,
  }),
);
...
let registerClient = $derived(createClient(ManagementService, authTransport)); // managementClient without token intersector for Register()
...
export let RegisterClient = () => registerClient;
