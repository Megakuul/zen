...
<div class="glass w-full min-h-[70vh] max-w-[1800px] p-8 sm:p-15 rounded-2xl flex flex-col gap-8 text-base sm:text-4xl">
  {#if user && edit}
    <input transition:fade bind:value={user.username} type="text" autocomplete="username" placeholder="Username" class="glass text-center p-3 sm:p-5 rounded-xl focus:outline-0" />
    <input transition:fade bind:value={user.email} type="email" autocomplete="email" placeholder="Email" class="glass text-center p-3 sm:p-5 rounded-xl focus:outline-0" />
    <input transition:fade bind:value={user.phone} type="tel" autocomplete="tel" placeholder="Phonenumber" class="glass text-center p-3 sm:p-5 rounded-xl focus:outline-0" />
    
    <button class="glass flex flex-row items-center">
  ...
  {:else}
    <h1 class="font-bold text-xl sm:text-3xl lg:text-6xl flex flex-row items-center gap-4">
    ...
    <p>Please reload the page and ensure you are <a class="underline" href="/login">Logged in</a></p>
    <div class="w-full h-20 bg-slate-500/10 rounded-2xl animate-pulse"></div>
    <div class="w-full h-20 bg-slate-500/10 rounded-2xl animate-pulse"></div>
    <div class="w-full h-20 bg-slate-500/10 rounded-2xl animate-pulse"></div>
  {/if}
</div>