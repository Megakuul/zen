...
func (o *Operator) deployEmail(ctx *pulumi.Context, input *emailInput) (*emailOutput, error) {
	if len(o.domains) < 1 {
	...
	emailIdentity, err := sesv2.NewEmailIdentity(ctx, "email", &sesv2.EmailIdentityArgs{
		EmailIdentity: pulumi.String(o.domains[0]),
		Region:        pulumi.String(o.region),
	...
	envelopeDomain := fmt.Sprintf("%s.%s", "bounce", o.domains[0])
	...
	if o.autoDns {
		envelopeZone, err := lookupZone(ctx, envelopeDomain)
		if err != nil {
			return nil, err
		}
		_, err = route53.NewRecord(ctx, "email-bounce", &route53.RecordArgs{
			...
			Records: pulumi.ToStringArray([]string{
				fmt.Sprintf("10 feedback-smtp.%s.amazonses.com", o.region),
			}),
		})
		if err != nil {
			return nil, err
		...
		zone, err := lookupZone(ctx, o.domains[0])
		if err != nil {
			return nil, err
		}
		emailIdentity.DkimSigningAttributes.Tokens().ApplyT(func(selectors []string) error {
			for i, selector := range selectors {
				_, err = route53.NewRecord(ctx, fmt.Sprintf("email-dkim-%d", i), &route53.RecordArgs{
					...
					Name:   pulumi.String(fmt.Sprintf("%s._domainkey.%s", selector, o.domains[0])),
		...
		_, err = route53.NewRecord(ctx, "email-dmarc", &route53.RecordArgs{
			ZoneId: pulumi.String(zone.Id),
			Name:   pulumi.String(fmt.Sprintf("_dmarc.%s", o.domains[0])),
			Type:   pulumi.String("TXT"),
			Records: pulumi.ToStringArray([]string{
		...
		ctx.Export(fmt.Sprintf("REQUIRED_MX_BOUNCE"), pulumi.Sprintf("MX %s \"%s\"",
			...
			pulumi.String(fmt.Sprintf("10 feedback-smtp.%s.amazonses.com", o.region)),
		))
		...
		emailIdentity.DkimSigningAttributes.Tokens().ApplyT(func(selectors []string) error {
			for i, selector := range selectors {
				ctx.Export(fmt.Sprintf("REQUIRED_CNAME_DKIM_%d", i), pulumi.Sprintf("CNAME %s %s",
					pulumi.String(fmt.Sprintf("%s._domainkey.%s", selector, o.domains[0])),
		...
		ctx.Export(fmt.Sprintf("REQUIRED_TXT_DMARC"), pulumi.Sprintf("TXT %s \"%s\"",
			pulumi.String(fmt.Sprintf("_dmarc.%s", o.domains[0])),
